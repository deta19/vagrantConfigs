...
sudo echo "GRANT ALL ON *.* TO ${DB_USER}@'127.0.0.1' IDENTIFIED BY '${DB_PASS}' WITH GRANT OPTION;" > /tmp/sql
sudo echo "GRANT ALL ON *.* TO ${DB_USER}@'localhost' IDENTIFIED BY '${DB_PASS}' WITH GRANT OPTION;" >> /tmp/sql
sudo echo "GRANT ALL ON *.* TO ${DB_USER}@'::1' IDENTIFIED BY '${DB_PASS}' WITH GRANT OPTION;" >> /tmp/sql
sudo echo "DELETE FROM mysql.user WHERE User='';" >> /tmp/sql
sudo echo "DROP DATABASE test;" >> /tmp/sql
sudo echo "FLUSH PRIVILEGES;" >> /tmp/sql
